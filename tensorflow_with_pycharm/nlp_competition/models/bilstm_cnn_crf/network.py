...
class Settings(object):
    def __init__(self):
        ...
        self.ckpt_path = self.root_path + '/ckpt/' + self.model_name + '/'
        self.summary_path = self.root_path + '/summary/' + self.model_name + '/'
...
class BiLstmCNNCRF(object):
    ...
    def cnn(self):
        # conv1d input:[batch_size, time_step, embedding_size] output:[batch_size, time_step, 50]
        cnn_input = tf.nn.dropout(self.char_embedding, self.dropout_prob)
        weights = self._variable_with_weight_decay(
            'cnn_weights', [self._cnn_kernel_size, self.embedding_size, self._cnn_kernel_outdim], 0)
        # kernel[5, hidden, 50]
        cnn_output = tf.nn.conv1d(cnn_input, weights, self._cnn_stride, padding='SAME', name='conv1d')
        cnn_output = tf.tanh(cnn_output, name='tanh')
        # max pooling
        # cnn_output = tf.layers.max_pooling1d(cnn_output, pool_size=[5], strides=[2], padding='same', name='pool')
        # dropout
        cnn_output = tf.nn.dropout(cnn_output, self.dropout_prob)
        return cnn_output
...
if __name__ == '__main__':
    _input = tf.Variable(tf.random_uniform(shape=[10, 128]), dtype=tf.float32)
    _input = tf.reshape(_input, [10, 128, 1])
    b = tf.Variable(tf.random_uniform(shape=[6, 1, 100]))
    conv1d = tf.nn.conv1d(_input, b, 1, padding='SAME')
